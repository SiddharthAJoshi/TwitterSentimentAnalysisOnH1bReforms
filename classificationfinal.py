...
clfr=LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, fit_intercept=True, intercept_scaling=1, solver='newton-cg', max_iter=100, verbose=0, warm_start=False, n_jobs=1,multi_class="multinomial")   # Initializing the Logistic Regression Classifier
clfr.fit(X_train,y_train)
resultr = clfr.predict(X_test)

print('-------------------------------------------------------------------------------------------------')
print('LogisticRegression Result')
print('The confusion matrix')
print(confusion_matrix(y_test,resultr))
print('The classification report')
print(classification_report(y_test,resultr))
print('The accuaracy score')
print(accuracy_score(y_test, resultr))
seriesr=pd.Series(resultr)
...
'''
lr_true = y_test
skplt.metrics.plot_confusion_matrix(lr_true, resultr)
plt.show()
skplt.estimators.plot_learning_curve(clfr, X, y)
plt.show()
data["LogisticRegression"]=seriesr
